...
def main():
    ...
    for g in ['human','ai']:
        s=frame.loc[frame.group.eq(g),'dated_promise']
    ...
    results['within_purpose_exploratory'] = {p: binary_comparison(s,'dated_promise') for p,s in frame.groupby('purpose')}
    ...
    if duration_path.exists() and response:
        ...
        family={'duration':duration['comparison']['permutation_pvalue'],'relevant_response':response['fisher_two_sided_p']}
        ordered=sorted(family,key=family.get); previous=0.; adjusted={}
        for rank,key in enumerate(ordered):
            previous=max(previous,min(1.,(len(ordered)-rank)*family[key])); adjusted[key]=previous
        results['secondary_holm_pvalues']=adjusted
